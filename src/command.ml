...
let rec sexp_of_poly ?(is_templ=false) sexp =
  ...
  | Sexp.List (Sexp.Atom op :: args) ->
      let args = List.map (sexp_of_poly ~is_templ) args in
      begin match op with
  ...
  | Sexp.List (Sexp.Atom command :: args) ->
      ...
      | "interpolant" ->
          if List.length args = 3 then
            ...
            let f1 = sexp_of_formula (List.nth args 0) in
            let f2 = sexp_of_formula (List.nth args 1) in
            let template = sexp_of_poly ~is_templ:true (List.nth args 2) in
            let d = Conv.int_of_sexp (List.nth args 3) in
            if
              List.exists
                (fun (_, c) -> Formula.PPoly.degree c > 1)
                (Formula.Poly.to_list template)
            then
              Conv.of_sexp_error "coefficients' polynomials must be linear polynomials" sexp
            else
